...
Recipe for turbo previews: `--sample_lora_overlay minimax_h3_turbo_4step_ema_ckpt850.safetensors` plus `sample_steps = 4` in the prompt file (the launcher wires `SAMPLE_LORA_OVERLAY=path[:strength]`). Below 8 steps the trainer automatically falls back from `ab2` to `euler`: AB2's multistep extrapolation overshoots on few-step shift-12 schedules (step ratios approach 2.0) and fries the output. Per prompt, `sample_lora_overlay = 0` in a `[[prompt.subset]]` opts that subset out, so one preview grid can carry turbo arms (fast, 4 steps) and clean arms (20 steps, no overlay) side by side. Keep checkpoint-judging renders on a clean arm — the Turbo LoRA changes texture statistics and is a confound, like EasyCache.
