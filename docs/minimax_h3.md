...
### Preview LoRA overlays (4-step previews with the Turbo LoRA)

`--sample_lora_overlay PATH[:STRENGTH]` (repeatable) stacks a frozen LoRA on the model during preview sampling only — most usefully the community [MiniMax-H3 Turbo LoRA](https://huggingface.co/larryvrh/MiniMax-H3-Turbo-Lora), which makes 4-step previews viable (~5x faster preview ticks, or affordable video previews). The overlay runs in activation space (`out += strength * up @ down @ x`) and is detached after sampling: nothing is merged into weights — required for INT8 base layers, and it avoids bf16 rounding that would eat the Turbo LoRA's tiny distill deltas — so training and saved checkpoints are byte-identical with or without it. Accepted key formats: musubi/kohya, ComfyUI (`diffusion_model.*.lora_A`), bare Turbo-node keys, and diffusers-PEFT exports like the [lightx2v turbo](https://huggingface.co/lightx2v/Minimax-h3-Turbo) (`transformer_blocks.*.lora_A.default.weight` — split to_q/to_k/to_v map onto the fused qkv via output slices, the diffusers SwiGLU half-swap on `ff.net.0.proj` is undone, and the reference `lora_alpha=8` is applied automatically, so run it at overlay strength 1.0).

Recipe for turbo previews: `--sample_lora_overlay minimax_h3_turbo_4step_ema_ckpt850.safetensors` plus `sample_steps = 4` in the prompt file (the launcher wires `SAMPLE_LORA_OVERLAY=path[:strength]`). Below 8 steps the trainer automatically falls back from `ab2` to `euler`: AB2's multistep extrapolation overshoots on few-step shift-12 schedules (step ratios approach 2.0) and fries the output. Per prompt, `sample_lora_overlay` in a `[[prompt.subset]]` takes a float that scales the configured overlay strength for that subset (`0` disables it, `0.8` runs it at 80%), so one preview grid can carry turbo arms at several strengths and clean arms (20 steps, no overlay) side by side; the launcher's `SAMPLE_LORA_OVERLAY_STRENGTH` sets the run-level base strength (turbo card guidance: `1.0` default, `0.8–0.95` against over-sharpening artifacts, `1.05–1.2` against blur). Keep checkpoint-judging renders on a clean arm — the Turbo LoRA changes texture statistics and is a confound, like EasyCache.

The Turbo LoRA includes adaln modules. On full-layout bases they overlay like everything else; on pruned (curve-table) bases the adaln update lives in a t-embedding space the model never materializes, so pass `--sample_lora_overlay_temb_grid` pointing at the `h3_silu_temb_grid.safetensors` that ships with the ComfyUI Turbo node (the launcher auto-detects it under `$COMFYUI_DIR/custom_nodes/`). Without the grid, adaln modules are skipped with a warning and the backbone overlay still applies.
