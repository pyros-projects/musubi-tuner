...
def text_encoder_cache_matches_caption(item: ItemInfo) -> bool:
    ...
    if cached_caption != item.caption:
        logger.info(f"Re-encoding text encoder cache with stale caption metadata: {item.text_encoder_output_cache_path}")
        return False
    return True
        ...
        for batch in tqdm(batches):
            ...
            if skip_existing:
                ...
                for item in batch:
                    ...
                    if cache_path not in all_cache_files:
                        filtered_batch.append(item)
                    elif not text_encoder_cache_matches_caption(item):
                        try:
                            os.remove(item.text_encoder_output_cache_path)
                        except FileNotFoundError:
                            pass
                        all_cache_files.discard(cache_path)
                        filtered_batch.append(item)
