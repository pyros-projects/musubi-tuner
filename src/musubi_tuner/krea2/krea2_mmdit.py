...
class SingleStreamDiT(nn.Module):
    ...
    def enable_block_swap(
        self, num_blocks: int, device: torch.device, supports_backward: bool, use_pinned_memory: bool = False
    ):
        ...
        print(
            ...
            f"to device {device}. Supports backward: {supports_backward}"
        )
    ...
    def override_block_swap_for_sampling(self, sample_blocks_to_swap: int | None, device: torch.device):
        ...
        if target_blocks_to_swap < 0:
            raise ValueError("Krea2 sample_blocks_to_swap must be a non-negative integer.")

        num_main_blocks = len(self.blocks)
        max_blocks_to_swap = num_main_blocks - 2
        if target_blocks_to_swap > max_blocks_to_swap:
            raise ValueError(f"Cannot swap more than {max_blocks_to_swap} Krea2 blocks during sampling. Requested {target_blocks_to_swap}.")
        ...
        if (original_blocks_to_swap > 0 or target_blocks_to_swap > 0) and self.offloader is None:
            raise ValueError("Krea2 sample_blocks_to_swap override requires initialized block swap when a positive count is involved.")
